...
OUTPUT_WORKPAPER = "货币supporting底稿_AI收口版.xlsx"
...
def build_ai_type_prompt(file_name: str) -> str:
    return f"""
你是审计 supporting 文件分类助手。请根据文件名判断该文件最可能属于哪一类：
...
def ai_guess_file_type(file_name: str) -> Tuple[str, float, str]:
    client = get_deepseek_client()
    prompt = build_ai_type_prompt(file_name)

    response = client.chat.completions.create(
        model="deepseek-chat",
        messages=[
            {"role": "system", "content": "你是一个严谨、克制、专业的审计文件分类助手。"},
    ...
    try:
        ...
        type_code = str(data.get("type", "unknown")).strip()
        if type_code not in ["M1", "M2", "M3", "unknown"]:
            type_code = "unknown"
        ...
        return type_code, confidence, reason
    except Exception:
        return "unknown", 0.0, f"AI输出解析失败：{text[:80]}"
...
def classify_and_rename_files(
    ...
    for file_path in source_path.iterdir():
        ...
        # 只有规则识别不到文件类型时，才调用 AI
        if not type_code and enable_ai_assist:
            ai_type_code, ai_confidence, ai_reason = ai_guess_file_type(file_name=file_name)
            ai_suggestion = f"{ai_type_code} ({ai_confidence:.2f})" if ai_type_code else ""
...
def run_supporting_pipeline(
    source_dir: str,
    output_dir: str = "supporting_output_ai_v2",
    ...
    return classified_df, workpaper_df, missing_df
...
if __name__ == "__main__":
    ...
    classified_df, workpaper_df, missing_df = run_supporting_pipeline(
        source_dir=SOURCE_DIR,
        output_dir="supporting_output_ai_v2",
        output_excel="货币supporting底稿_AI收口版.xlsx",
